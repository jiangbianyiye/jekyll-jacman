...
d.k值应当与归并排序与插入排序的常数因子有关。这样考虑：该思考题给出的算法比一般归并算法快，是应为在得到的子表足够小时，将原本应当继续使用递归进行归并排序的部分用常数因子更小的插入排序替代。为了简化问题，假设插入排序的时间为$$c_1n^2$$，归并排序的时间为$$c_2nlg(n)+c_2n$$考虑在得到n/k个长度为k的子表后，继续使用一般的归并排序算法所需的时间为$$c_2n(lg(n)-lg(n/k))$$ 而使用插入排序处理相同的部分则需要的时间最多为$$c_1k^2*n/k$$,令两式相等，得到$$c_1k=c_2lg(k)$$,解出该k值即为所需。然而实际情况复杂多样，与机器性能，使用的语言等均有关系，故应根据具体环境尝试不同的k值，与一般的归并排序进行比较得到合理的k值。